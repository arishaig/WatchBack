...
{
    internal static readonly TimeSpan SearchCacheDuration = TimeSpan.FromMinutes(10);
    internal static readonly TimeSpan EpisodeCacheDuration = TimeSpan.FromHours(24);

    // Shared brand data used by both the search and ratings provider metadata
    private static readonly BrandData OmdbBrandData = new(
        Color: "#626f53",
    ...
    public Task<ServiceHealth> GetServiceHealthAsync(CancellationToken ct = default)
    {
        ...
        return Task.FromResult(new ServiceHealth(
            ...
            Message: configured ? "API key configured" : "No API key configured",
