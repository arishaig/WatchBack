...
/// <param name="SourceResults">
///     The aggregated results from each ThoughtProvider, including post metadata and Thought
///     collections
/// </param>
/// <param name="WatchProvider">Name of the watch state provider that supplied the current media context</param>
/// <param name="SuppressedProvider">
///     Name of the configured provider that has active context but was overridden by the
///     manual provider
/// </param>
...
{
    public static SyncResult Idle(int timeMachineDays)
    {
        return new SyncResult(
            SyncStatus.Idle, null, null, [], [], timeMachineDays, []);
    }

    public static SyncResult Error(int timeMachineDays)
    {
        return new SyncResult(
